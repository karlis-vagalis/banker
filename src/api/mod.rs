...
impl EnableBankingClient {
    pub async fn new(config: &Config, app: &ApplicationConfig) -> Result<Self, AppError> {
        ...
        let reqwest_client = reqwest::ClientBuilder::new()
            ...
            .build()?;
        let base_url = config.api_base_url.as_str().trim_end_matches('/');
    ...
    pub async fn list_aspsps(
        &self,
        country: Option<&openapi::types::Country>,
    ) -> Result<Vec<openapi::types::AspspData>, AppError> {
        let resp = api_err(
            self.client
                .get_aspsps_aspsps_get(country, None, None, None)
                .await,
        )?;
        ...
        api_err(
            self.client
                .delete_session_sessions_session_id_delete(
                    session_id, None, None, None, None, None, None, None, None,
                )
                .await,
        )?;
        Ok(())
        ...
        let resp = api_err(
            self.client
                .get_account_accounts_account_id_details_get(
                    account_id, None, None, None, None, None, None, None, None,
                )
                .await,
        )?;
        Ok(resp.into_inner())
    ...
    pub async fn get_account_transactions(
        ...
        if let Ok(sessions) = Sessions::load(&session_path())
            && let Some(session) = sessions.find_by_account_id(account_id)
        {
            ...
            if matches!(
                ...
                let may_include_old_data = time_frame.is_none_or(|tf| tf.from < cutoff);
